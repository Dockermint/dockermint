...
[docs/specs/pebbledb.md](../specs/pebbledb.md).
...
- The three `replace` directives are independent. Apply only the ones that
  match modules actually imported by the chain. To check:
...
(`db_backend = "pebbledb"`) and either:

1. Re-sync from genesis or a recent block height.
2. Import a PebbleDB-format snapshot.
3. Convert an existing `goleveldb` database with
   [Pebblify](https://github.com/Dockermint/pebblify).

The two backends are not on-disk compatible. Dockermint produces the binary
with the correct backend compiled in, but does not perform database conversion.
...
selects it, either through CLI arguments, `config.toml`, or the recipe defaults.
...
Flavor resolution follows CLI arguments > `config.toml` > recipe defaults.
...
with native support omit the `[[pre_build]]` patching steps entirely; the
builder requires only build tags and linker flags.

### Recipe TOML structure for patching (older chains)
...
Only include the `go mod edit -replace` directives that match modules actually
...
covers both `goleveldb` and `pebbledb` without conditional Rust logic:
